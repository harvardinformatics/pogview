...
public class PiRenderer implements RendererI {
            
    public static boolean sizeByInfo = false;
  ...
  public void drawLogo(Graphics2D g, int xstart, int ystart, int charwidth , int charheight, TFMatrix matrix, int strand,String topstr) {
	int w;
	int h;
	...
	for (int i = 0; i < len; i++) {
		...
		ch      -= drawLetter(g2,str,charfont,c,frc,xstart+(int)(i*charwidth),(ystart+ch));
		ii++;
	    }
	}
    }
	...
	for (int i = 0; i < feat.size(); i++) {
	  ...
	  if (sf.getType().equals("30mamm.pi")) {
	    if (width >= 3) {
	      ...
	      while (ii < cols) {
		Vector v = sf.getScoreVectorAt(start + ii);
	      	vals[ii][0] = (Double.valueOf((Double)v.elementAt(0)));
	      	vals[ii][1] = (Double.valueOf((Double)v.elementAt(1)));
	      	vals[ii][2] = (Double.valueOf((Double)v.elementAt(2)));
	      	vals[ii][3] = (Double.valueOf((Double)v.elementAt(3)));
	      	ii++;
	      }
	    ...
	    if (width >= 3) {
	      drawLogo(g,tmpstart,y1,(int)width,height,mat,sf.getStrand(),str);
	    } else {
	      g.setColor(Color.lightGray);
	      g.fillRect(tmpstart,y1,(int)((sf.getEnd()-sf.getStart()+1)*width),height);
	    }
	    ...
	    g.setFont(f);
	    //g.drawString(sf.getId()+ "." + String.valueOf((int)sf.getScore()),tmpstart,y1+height);
