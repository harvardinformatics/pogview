...
public final class Hit {
    ...
    private static final Regex reg_score       = new Regex("Score =\\s+(\\S+) bits \\((\\d+)");
    private static final Regex reg_identities  = new Regex("Identities = (\\d+)\\/(\\d+)");
    private static final Regex reg_positives   = new Regex("Positives = (\\d+)");
    private static final Regex reg_pvalue      = new Regex("Expect(\\(\\d+\\))? =\\s+(\\S+)");
    private static final Regex reg_warn        = new Regex("^WARNING: |^NOTE:");
    private static final Regex reg_strand_hsp  = new Regex("Strand HSP");
    private static final Regex reg_strand      = new Regex("^\\s*Strand");
    private static final Regex reg_score2      = new Regex("^\\s*Score");
    private static final Regex reg_end_data    = new Regex("^>|^Parameters|^\\s+Database:|^CPU\\stime");
    private static final Regex reg_null        = new Regex("\\S");
    private static final Regex reg_frame       = new Regex("^\\s*Frame\\s*=\\s*([\\-\\+])(\\d+)");
    private static final Regex reg_query       = new Regex("^Query:\\s+(\\d+)\\s*([\\D\\S]+)\\s+(\\d+)");
    private static final Regex reg_hit         = new Regex("^Sbjct:\\s+(\\d+)\\s*([\\D\\S]+)\\s+(\\d+)");
    ...
    private BPLite parent;
    
    private boolean all_parsed = false;
...
//          System.out.println("Next " + next_line);
        ...
        HSP hsp = new HSP(new SequenceFeature(),new SequenceFeature());
